...
use {
    crate::{block_cost_limits::*, instruction_details::InstructionDetails, transaction_cost::*},
    log::*,
...
impl CostModel {
    ...
    pub fn calculate_cost_sum(
        ...
        let program_execution_cost = instruction_details.programs_execution_cost;
        let loaded_accounts_data_size_cost =
            if feature_set.is_active(&include_loaded_accounts_data_size_in_fee_calculation::id()) {
                FeeStructure::calculate_memory_usage_cost(
                    usize::try_from(instruction_details.loaded_accounts_bytes).unwrap(),
                    DEFAULT_HEAP_COST,
                )
            } else {
                0
        ...
        let data_bytes_cost =
            instruction_details.instruction_data_bytes / INSTRUCTION_DATA_BYTES_COST;
        let write_lock_cost = WRITE_LOCK_UNITS.saturating_mul(transaction.num_write_locks());

        signature_cost
            .saturating_add(program_execution_cost)
