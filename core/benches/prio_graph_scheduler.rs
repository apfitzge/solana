...
fn create_conflicting_transactions(num: usize) -> Vec<SanitizedTransaction> {
    let accounts = create_accounts(2 * num);
    let from = &accounts[0];
    accounts[1..]
        .into_par_iter()
        .map(|to| system_transaction::transfer(from, &to.pubkey(), 1, Hash::default()))
        .map(SanitizedTransaction::from_transaction_for_tests)
        .collect()
}
...
fn bench_schedule_conflicting(bencher: &mut Bencher, batch_size: usize) {
    const NUM_WORKERS: usize = 1;
    const TRANSACTIONS_PER_ITERATION: usize = 10240;
    assert_eq!(
        TRANSACTIONS_PER_ITERATION % batch_size,
    ...
    // Each iteration has a new container. This is so we can avoid the cost of filling the container
    // in the benchmark time.
    let transactions = create_conflicting_transactions(2_usize.pow(24));
    let containers: Vec<_> = transactions
        .into_iter()
        .chunks(batch_size)
        .into_iter()
...
#[bench]
fn bench_schedule_conflicting_1024(bencher: &mut Bencher) {
    bench_schedule_conflicting(bencher, 1024);
}
