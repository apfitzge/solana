...
mod tests {
    ...
    fn test_worker_consume_no_bank() {
        ...
        assert_eq!(consumed.work.transaction_ids, vec![id]);
    ...
    fn test_worker_consume_self_conflicting() {
        ...
        let bid = TransactionBatchId::new(0);
        let id1 = TransactionId::new(1, 0);
        let id2 = TransactionId::new(0, 0);
        consume_sender
            .send(ConsumeWork {
                batch_id: bid,
                transaction_ids: vec![id1, id2],
                transactions: txs,
            })
            .unwrap();
        ...
        let consumed = consumed_receiver.recv().unwrap();
        assert_eq!(consumed.work.batch_id, bid);
        assert_eq!(consumed.work.transaction_ids, vec![id1, id2]);
    ...
    fn test_worker_consume_multiple_messages() {
        ...
        consume_sender
            .send(ConsumeWork {
                batch_id: bid2,
                transaction_ids: vec![id2],
                transactions: txs2,
            })
            .unwrap();
        let consumed = consumed_receiver.recv().unwrap();
        assert_eq!(consumed.work.batch_id, bid1);
        assert_eq!(consumed.work.transaction_ids, vec![id1]);
        assert_eq!(consumed.retryable_indexes, Vec::<usize>::new());

        let consumed = consumed_receiver.recv().unwrap();
        assert_eq!(consumed.work.batch_id, bid2);
        assert_eq!(consumed.work.transaction_ids, vec![id2]);
        assert_eq!(consumed.retryable_indexes, Vec::<usize>::new());
