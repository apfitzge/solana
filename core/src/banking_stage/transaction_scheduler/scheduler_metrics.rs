...
impl SchedulingDetails {
    pub fn update(&mut self, scheduling_summary: &SchedulingSummary) {
        ...
        self.sum_unschedulable_conflicts += scheduling_summary.num_unschedulable;
        self.sum_throttled += scheduling_summary.num_throttled;
    }
        ...
        if self.last_report.duration_since(now) > REPORT_INTERVAL {
            ...
            if self.num_schedule_calls > 0 {
                ...
                datapoint_info!(
                    ...
                    ("num_unschedulable", self.sum_unschedulable_conflicts, i64),
                    ("num_throttled", self.sum_throttled, i64),
                );
                *self = Self {
                    last_report: now,
                    ..Self::default()
