...
pub struct SnapshotStorageRebuildResult {
    /// Unpacked, remapped, and read storage entries by slot
    pub storage: DashMap<Slot, SlotStores>,
    /// Next AppendVecId
    pub next_append_vec_id: AtomicU32,
}
...
impl SnapshotStorageRebuilder {
    ...
    fn spawn_rebuilder_threads(
        ...
        let rebuilder = Arc::try_unwrap(rebuilder).unwrap();
        SnapshotStorageRebuildResult {
            storage: rebuilder.storage,
            next_append_vec_id: rebuilder.next_append_vec_id,
        }
    }
